...
const ProjectDetails = () => {
  ...
  const chosenProj = ProjectsData.find(
    (project) => project.name === decodedName
  )
  ...
  if (!chosenProj) {
    return (
      <main className="projectData project-not-found">
        ...
        <Link to="/#proyectos" className="detail-link">
          Volver a proyectos
        </Link>
      </main>
    )
  ...
  return (
    <main className="projectData">
      ...
      <section
        ...
        aria-label="Resumen profesional del proyecto"
      >
        <article>
      ...
      <section className="project-responsibilities">
        ...
        <h2>
          Ejecución, supervisión y calidad en obra.
        </h2>

        <ul>
          {chosenProj.responsibilities.map(
            (responsibility) => (
      ...
      <section
        ...
        aria-label={`Galería editorial de ${chosenProj.name}`}
      >
        {supportingMedia.map((item, index) => (
          <figure
            key={`${item.name}-${index}`}
            className={`${index % 4 === 0 ? 'wide' : index % 4 === 1 ? 'tall' : ''} ${
              item.type === 'video'
                ? 'video-card'
                : ''
            }`.trim()}
